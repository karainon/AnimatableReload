...
protocol ReloadAnimationHostView {

    var indexPathsForVisibleItems: [IndexPath] { get }

    var visibleSectionIndexes: [Int: HeaderFooterTuple] { get }

    var frame: CGRect { get }

    func item(at: IndexPath) -> UIView?
...
public enum AnimationType {
    ...
    func animate(view: ReloadAnimationHostView, reversed: Bool = false, completion: (() -> Void)? = nil) {
        ...
        let indexPathsForVisibleItems = view.indexPathsForVisibleItems
        let grouped = indexPathsForVisibleItems
            .sorted(by: { (one, two) -> Bool in one.row < two.row })
            .grouped(by: { (indexPath: IndexPath) -> Int in return indexPath.section })
            .sorted(by: { $0.key < $1.key })

        let visibleHeaderFooter = view.visibleSectionIndexes
        var visibleViews = [UIView]()

        for items in grouped {
            var currentViews: [UIView] = items.value.compactMap { view.item(at: $0) }
        ...
        cells.enumerated().forEach { item in
            ...
            direction.startValues(frame: view.frame, for: item.element)
            ...
            UIView.animate(
                ...
                animations: {
                    direction.endValues(frame: view.frame, for: item.element)
...
public enum Direction {
    ...
    func startValues(frame: CGRect, for cell: UIView) {
        cell.alpha = 0
        switch self {
        case .left(let useCellsFrame):
            cell.frame.origin.x += useCellsFrame ? cell.frame.width : frame.width
        case .top(let useCellsFrame):
            cell.frame.origin.y += useCellsFrame ? cell.frame.height : frame.height
    ...
    func endValues(frame: CGRect, for cell: UIView) {
        cell.alpha = 1
        switch self {
        case .left(let useCellsFrame):
            cell.frame.origin.x -= useCellsFrame ? cell.frame.width : frame.width
...
extension UITableView: ReloadAnimationHostView {
    
    var indexPathsForVisibleItems: [IndexPath] { return self.indexPathsForVisibleRows ?? [] }

    var visibleSectionIndexes: VisibleHeaderFooter {
    ...
    func item(at: IndexPath) -> UIView? {
        return self.cellForRow(at:at)
    }
}

extension UICollectionView: ReloadAnimationHostView {
    fileprivate var header: String { return UICollectionView.elementKindSectionHeader }
    fileprivate var footer: String { return UICollectionView.elementKindSectionFooter }
    
    var visibleSectionIndexes: VisibleHeaderFooter {
        var visibleHeaderFooter: VisibleHeaderFooter = [:]

        indexPathsForVisibleSupplementaryElements(ofKind: header).forEach { (indexPath) in
            let tuple: HeaderFooterTuple = (header: supplementaryView(forElementKind: header, at: indexPath), footer: nil)
            visibleHeaderFooter[indexPath.section] = tuple
        }
        
        indexPathsForVisibleSupplementaryElements(ofKind: footer).forEach { (indexPath) in
            var tuple: HeaderFooterTuple = visibleHeaderFooter[indexPath.section] ?? (header: nil, footer: nil)
            tuple.footer = supplementaryView(forElementKind: footer, at: indexPath)
            visibleHeaderFooter[indexPath.section] = tuple
        }
